...
addLayer("r", {
            ...
            Energy: {
                content: [
                    ...
                    ["display-text",function() {
                        ...
                        if(player.r.bEnergy.eq(0)) resourceText = ""
                        let effText = [
                    ...
                    ["display-text",function() {
                        ...
                        if(player.r.gEnergy.eq(0)) resourceText = ""
                        let effText = [
                    ...
                    ["display-text",function() {
                        ...
                        if(player.r.rEnergy.eq(0)) resourceText = ""
                        let effText = [
    ...
    color: "#ff0000",
    requires() {
        let req = new Decimal(15e6)
        if(getBuyableAmount("r",13).gte(14)) req = req.div(tmp.r.rEnergyEff2)
        return req
    ...
    gainMult() { // Calculate the multiplier for main currency from bonuses
        ...
        if(hasMilestone("r",7)) mult = mult.add(1)
        if(hasUpgrade("b",53)) mult = mult.add(1)
        mult = mult.add(tmp.r.rEnergyEff1)
        if(hasUpgrade("r",11)) mult = mult.add(10)
        if(hasUpgrade("r",12)) mult = mult.add(10)
        if(hasUpgrade("r",13)) mult = mult.add(10)
        if(hasUpgrade("r",14)) mult = mult.add(10)
        if(hasUpgrade("r",15)) mult = mult.add(10)
        if(hasUpgrade("r",21)) mult = mult.add(20)
        if(hasUpgrade("r",22)) mult = mult.add(20)
        if(hasUpgrade("r",23)) mult = mult.add(20)
        if(hasUpgrade("r",24)) mult = mult.add(20)
        if(hasUpgrade("r",25)) mult = mult.add(20)
        return mult
    },
    ...
    redM4eff() {
        let inc = player.r.points.mul(5000)
        return inc
    },
    ...
    bEnergyEff1() {
        ...
        if(hasMilestone("r",8)) mult = mult.mul(2)
        if(hasUpgrade("g",64)) mult = mult.mul(2)
        return mult
    },
    ...
    bEnergyEff2() {
        let be = player.r.bEnergy
        if(getBuyableAmount("r",12).gte(10)) be = be.add(tmp.r.gEnergyEff3)
        let mult = be.sub(3).div(4).add(1).max(1)
        if(hasUpgrade("g",71)) mult = mult.mul(1.5)
        if(hasUpgrade("r",24)) mult = mult.mul(2)
        if(hasUpgrade("r",33)) mult = mult.mul(1.3)
        return mult
    },
    ...
    bEnergyEff4() {
        ...
        let mult = be.sub(44).pow(1.2).mul(1e11)
    ...
    bEnergyEff5() {
        let be = player.r.bEnergy
        let mult = be.sub(120).pow(1.5).mul(0.3)
        return mult
    },
    ...
    gEnergyEff4() {
        let ge = player.r.gEnergy
        if(getBuyableAmount("r",13).gte(16)) ge = ge.add(tmp.r.rEnergyEff3)
        let mult = ge.sub(22).pow(1.4).mul(1e13)
    ...
    rEnergyEff2() {
        let mult = new Decimal(5).add(player.r.rEnergy.sub(14).max(0).pow(1.25)).mul(3).max(1)
        return mult
    },

    rEnergyEff3() {
        let inc = player.r.rEnergy.sub(19)
    ...
    canReset() {return !hasMilestone("r",0)&&player.points.gte(tmp.r.requires)&&hasUpgrade("g",45)},
    ...
    buyables: {
        11: {
            title: "Red #B1",
            cost(x) {
                let base = [new Decimal(36e4), new Decimal(2.5e6), new Decimal(5e7),new Decimal(1e15)]
                let scale = [new Decimal(1e4),new Decimal(5e4), new Decimal(10e4), new Decimal(2e6), new Decimal(1e8),new Decimal(1.02)]
                let scaledCost = [
                    base[0].add(scale[0].mul(x)),
                    base[0].add(scale[0].mul(18)).add(scale[1].mul(x.sub(18))),
                    base[1].add(scale[2].mul(x.sub(40))),
                    base[2].add(scale[3].mul(x.sub(120).pow(1.1))),
                    base[2].add(scale[3].mul(new Decimal(380).pow(1.1))).add(scale[4].mul(x.sub(500).pow(1.5))),
                    base[3].mul(scale[4].pow(x.sub(1000))),
                ]
                let consume = scaledCost[0]
                if(x.gte(19)&&x.lt(40)) consume = scaledCost[1]
                if(x.gte(40)&&x.lt(120)) consume = scaledCost[2]
                if(x.gte(120)&&x.lt(500)) consume = scaledCost[3]
        ...
        12: {
            title: "Red #B2",
            cost(x) {
                let base = [new Decimal(6e10), new Decimal(3e15)]
                let scale = [new Decimal(1.5), new Decimal(1.7)]
                let accel = new Decimal(1.022)
                let scaledCost = [
                    scale[0].pow(x.pow(accel)).mul(base[0]),
                    scale[0].pow(x.pow(accel)).mul(base[0]).div(10),
                    scale[1].pow(x.sub(20).pow(accel)).mul(base[1]),
                    scale[1].pow(x.sub(20).pow(accel)).mul(base[1]).div(10),
                ]
                let consume = scaledCost[0]
                if(x.gte(1)&&x.lt(20)) consume = scaledCost[1]
                if(x.gte(20)&&x.lt(21)) consume = scaledCost[2]
                if(x.gte(21)) consume = scaledCost[3]
                return consume
            },
            baseEffect() {
                let base = new Decimal(1)
        ...
        13: {
            ...
            cost(x) {
                ...
                let scaledCost = [
                    ...
                    scale[0].pow(x.pow(accel)).mul(base).div(10),
                ]
                let consume = scaledCost[0]
                if(x.gte(1)) consume = scaledCost[1]
                return consume
            },
            baseEffect() {
                let base = new Decimal(1)
    ...
    upgrades: {
        11: {
            title: "Red #1",
            description: "+10 RE mult, BEG1 gives a small multiplier to BEG4.",
            effect() {
                let eff = tmp.r.bEnergyEff1.pow(0.25).add(0.5)
                return eff
            },
            effectDisplay() {return "x"+format(this.effect())},
            cost: new Decimal(10e4),
            unlocked() {return hasMilestone("r",14)},
            style: {'touch-action':'manipulation'},
        },
        12: {
            title: "Red #2",
            description: "+10 RE mult, the effect of Blue #B3 is altered.",
            cost: new Decimal(12e4),
            unlocked() {return hasUpgrade("r",11)},
            style: {'touch-action':'manipulation'},
        },
        13: {
            title: "Red #3",
            description: "+10 RE mult, the effect of Green #17 is altered.",
            cost: new Decimal(20e4),
            unlocked() {return hasUpgrade("r",12)},
            style: {'touch-action':'manipulation'},
        },
        14: {
            title: "Red #4",
            description: "+10 RE mult, the effect of Green #20 is altered.",
            cost: new Decimal(30e4),
            unlocked() {return hasUpgrade("r",13)},
            style: {'touch-action':'manipulation'},
        },
        15: {
            title: "Red #5",
            description: "+10 RE mult, the requirement of getting next GE is reduced by 50%.",
        ...
        21: {
            title: "Red #6",
            description: "+20 RE mult, GEG3 also applies to BEG4 and is 50% stronger.",
            cost: new Decimal(100e4),
            unlocked() {return hasUpgrade("r",15)},
            style: {'touch-action':'manipulation'},
        },
        22: {
            title: "Red #7",
            description: "+20 RE mult, BEG3 gives a small multiplier to GEG4.",
        ...
        23: {
            title: "Red #8",
            description: "+20 RE mult, RE passive gain and Red #M8 effect are twice as strong.",
            cost: new Decimal(200e4),
            unlocked() {return hasUpgrade("r",22)},
            style: {'touch-action':'manipulation'},
        },
        24: {
            title: "Red #9",
            description: "+20 RE mult, BEG2 is twice as strong.",
            cost: new Decimal(400e4),
            unlocked() {return hasUpgrade("r",23)},
            style: {'touch-action':'manipulation'},
        },
        25: {
            title: "Red #10",
            description: "+20 RE mult, Blue #B5 no longer costs BE. Unlock a new Blue buyable.",
            cost: new Decimal(600e4),
        ...
        33: {
            title: "Red #13",
            description: "REG1 is twice as strong. BEG2 is 30% stronger.",
    ...
    milestones: {
        0: {
            ...
            effectDescription: "+50,000 points gain, GL gain and +1,000 BE cap. Disable the prestige button for Red and instead generate 10% RE on reset.",
        ...
        3: {
            requirementDescription: "Red #M4: 80 RE",
            effectDescription() {
                let descText = "+1 RE mult. RE boosts points gain significantly.<br>"
                let effText = "Currently: +"+format(tmp.r.redM4eff)
                return descText+effText
            },
            done() {return player.r.points.gte(80)},
        },
        4: {
            requirementDescription: "Red #M5: 120 RE",
            effectDescription: "Unlock a new row of Green upgrades.",
            done() {return player.r.points.gte(120)},
        },
        5: {
            requirementDescription: "Red #M6: 555 RE",
            effectDescription: "Unlock Energy.",
            done() {return player.r.points.gte(555)&&hasUpgrade("g",55)},
            unlocked() {return hasUpgrade("g",55)}
        },
        6: {
            requirementDescription: "Red #M7: 5 Green Energy",
            effectDescription: "Blue #15 uses the best Green Light you have instead of current one. Double GL gain.",
            done() {return player.r.gEnergy.gte(5)},
            unlocked() {return hasMilestone("r",5)}
        },
        7: {
            requirementDescription: "Red #M8: 1,600 RE",
            effectDescription() {
                let descText = "+1 RE mult, increase BE cap based on RE.<br>"
                let effText = "Currently: +"+format(tmp.r.redM8eff)
                return descText+effText
            },
            done() {return player.r.points.gte(1600)},
            unlocked() {return player.r.bEnergy.gte(11)}
        },
        8: {
            requirementDescription: "Red #M9: 8 Green Energy",
            effectDescription() {
                let exp = 0.25
                if(hasUpgrade("g",73)) exp = exp+0.1
                let descText = "BE^"+format(exp)+" multiplies GL gain. BEG1 and GEG1 is doubled.<br>"
                let effText = "Currently: x"+format(tmp.r.redM9eff)
                return descText+effText
            },
            done() {return player.r.gEnergy.gte(8)},
        ...
        11: {
            requirementDescription: "Red #M12: 30 Green Energy",
        ...
        12: {
            requirementDescription: "Red #M13: 65 Blue Energy",
            effectDescription: "Buying Blue Energy no longer costs BE.",
            done() {return player.r.bEnergy.gte(65)},
            unlocked() {return hasMilestone("r",11)}
        },
        13: {
            requirementDescription: "Red #M14: 40 Green Energy",
            effectDescription: "Buying Green Energy no longer costs GL.",
            done() {return player.r.gEnergy.gte(40)},
            unlocked() {return hasMilestone("r",11)}
        },
        14: {
            requirementDescription: "Red #M15: 120 Blue Energy & 12 Red Energy",
            effectDescription: "Unlock Red upgrades.",
            done() {return player.r.bEnergy.gte(120)&&player.r.rEnergy.gte(12)},
            unlocked() {return hasUpgrade("g",75)}
        },
        15: {
            requirementDescription: "Red #M16: 60 Green Energy",
            effectDescription: "All GEGs are twice as strong (not including the upcoming ones).",
            done() {return player.r.gEnergy.gte(60)},
            unlocked() {return hasUpgrade("r",25)}
        },
        16: {
            requirementDescription: "Red #M17: 300 Blue Energy",
            effectDescription() {
                let descText = "Drastically improve GEG2 based on BEG5.<br>"
                let effText = "Currently: x"+format(tmp.r.redM17eff)
                return descText+effText
            },
            done() {return player.r.bEnergy.gte(300)},
            unlocked() {return hasUpgrade("r",25)}
        },
        17: {
